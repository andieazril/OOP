...
public class SalesItem {
    ...
    public boolean addComment(String author, String text, int rating) {
        ...
        if (findCommentByAuthor(author) != null) {
            // reject multiple comments by the same author
            return false;
        }
    ...
    private boolean ratingInvalid(int rating) {
        return rating < 1 || rating > 5; // fixed the condition for invalid ratings
    }
    ...
    private String priceString(int price) {
        ...
        int cents = price - (dollars * 100); // fixed the subtraction logic
        if (cents <= 9) {
